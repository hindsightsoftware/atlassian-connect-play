 ...
 *     <li>Subsequent requests without OAuth headers can be verified by calling {@link #validate(TokenKey, String, long)} (see {@link CheckValidToken}</li>
 ...
 * </ul>
 * <p/>
 * Tokens have a lifetime (configurable via {@link com.atlassian.connect.play.java.Constants#AC_TOKEN_EXPIRY}).  Every time a token is validate, this lifetime is extended.  Once no requests are retrieved
 ...
 * custom token stores via the {@link com.atlassian.connect.play.java.Constants#AC_TOKEN_STORE} property (to use something like Redis for example).
 */
